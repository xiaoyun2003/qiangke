...
public class ZF {
    ...
    public User Login(String user,String pwd){
        ...
        BigInteger mb = new BigInteger(Base64.decode(m,Base64.NO_WRAP));
        BigInteger eb = new BigInteger(Base64.decode(e,Base64.NO_WRAP));
        String epwd;
            //x509编码
            ...
            out.close();
            epwd=new String(Base64.encode(encryptedData, Base64.NO_WRAP));
...
public CourseXK[] getCoursesXKByHomeData(HashMap<String,String> map){
    ...
    public ClassXK[] getClassesXK(User u,CourseXK cxk,int kspage,int jspage){
        try {
            ...
            String data="rwlx=" + rwlx + "&xkly=0&bklx_id=" + bklx_id + "&sfkkjyxdxnxq=0&xqh_id=1&jg_id=" + jg_id + "&njdm_id_1=" + njdm_id_1 + "&zyh_id_1=" + zyh_id_1 + "&zyh_id=" + zyh_id + "&zyfx_id=wfx&njdm_id=" + njdm_id + "&bh_id=" + bh_id + "&xbm=1&xslbdm=" + xslbdm + "&ccdm=" + ccdm + "&xsbj=" + xsbj + "&sfkknj=0&sfkkzy=0&kzybkxy=0&sfznkx=0&zdkxms=0&sfkxq=0&sfkcfx=0&kkbk=0&kkbkdj=0&sfkgbcx=0&sfrxtgkcxd=0&tykczgxdcs=0&xkxnm=" + xkxnm + "&xkxqm=" + xkxqm + "&kklxdm=" + kklxdm + "&bbhzxjxb=0&rlkz=0&xkzgbj=0&kspage="+(kspage)+"&jspage="+(jspage)+"&jxbzb=";
            ...
            for(int i=0;i<JSA.size();i++){
                ...
                clxk.xf=JS1.getString("xf");
                clxk.yxzrs=JS1.getString("yxzrs");
                classesxk[i]=clxk;
            }
            return classesxk;
        }catch(Exception e){
    ...
    public JXBXK[] getJXBsXK(User u,ClassXK cxk){
        try{
            ...
            for(int i=0;i<JSA.size();i++){
                ...
                jxbxk.jxb_id=JS1.getString("jxb_id");
...
return res;
    ...
    //简化版算法
    public SimpleClassXK[] getAllClassesXK(User u,CourseXK cxk,int kspage,int jspage){
        try {
            ClassXK[] cls = this.getClassesXK(u, cxk, kspage, jspage);
            SimpleClassXK[] scls = new SimpleClassXK[cls.length];
            for (int i = 0; i < cls.length; i++) {
                JXBXK[] jxbs = this.getJXBsXK(u, cls[i]);
                SimpleClassXK scl=new SimpleClassXK();
                int flag=1;
                for (JXBXK jxb : jxbs) {
                    if (cls[i].jxb_id.equals(jxb.jxb_id) && flag == 1) {
                        scl.do_jxb_id = jxb.do_jxb_id;
                        scl.jxb_id = jxb.jxb_id;
                        scl.jsxx = jxb.jsxx;
                        scl.jxbrl = jxb.jxbrl;
                        scl.jxdd = jxb.jxdd;
                        scl.jxms = jxb.jxms;
                        scl.kclbmc = jxb.kclbmc;
                        scl.kcxzmc = jxb.kcxzmc;
                        scl.kkxymc = jxb.kkxymc;
                        scl.fxbj = jxb.fxbj;
                        scl.jgpxzd = jxb.jgpxzd;
                        scl.year = jxb.year;
                        scl.sksj = jxb.sksj;
                        scl.xqumc = jxb.xqumc;
                        scl.kch_id = jxb.kch_id;
                        scl.kklxdm = jxb.kklxdm;
                        flag++;
                    }
                }
                scl.xf=cls[i].xf;
                scl.jxb_id = cls[i].jxb_id;
                scl.jxbmc = cls[i].jxbmc;
                scl.jxbzls = cls[i].jxbzls;
                scl.kch = cls[i].kch;
                scl.kch_id = cls[i].kch_id;
                scl.kcmc = cls[i].kcmc;
                scl.kcrow = cls[i].kcrow;
                scl.kklxdm = cls[i].kklxdm;
                scl.kzmc = cls[i].kzmc;
                scl.rwlx = cls[i].rwlx;
                scl.xkkz_id = cls[i].xkkz_id;
                scl.yxzrs=cls[i].yxzrs;
                scls[i]=scl;
            }
            return scls;
        }catch (Exception e){
